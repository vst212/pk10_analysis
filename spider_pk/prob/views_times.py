...
def admin(request):
    ...
    result_flag = True
    return render_to_response('index_times.html',{'lottery':lotterys,'probs':probs, 'prob_totals':prob_totals,'result_flag':result_flag})
...
def index(request):
    ...
    probs = Probs.objects.all()
    prob_totals = ProbTotals.objects.all()
    return render_to_response('index_times.html',{'lottery':lotterys,'probs':probs,'prob_totals':prob_totals,
                                            'p_date':p_date, 'p_number':p_number, 'p_monery':p_monery,'p_rule':p_rule, 'result_flag':result_flag, 'p_times':p_times})
...
def parase_lotterys(lottery):
    ...
    base_lottery_list = [[0 for i in range(10)] for i in range(179)]
    #定义行列矩阵,179行，10列，奇偶数组，1表示奇，0表示偶
    parity_lottery_list = [[0 for i in range(10)] for i in range(179)]
    #定义行列矩阵,179行，10列，大小数组,1表示大，0表示小
    larsma_lottery_list = [[0 for i in range(10)] for i in range(179)]
    count = 0
    for loty in lottery:
...
def compute_rule_old(times, num, rule, target, prob_value):
    count = 0
    index = 0
    max = len(target)- num
    while(count < max):
